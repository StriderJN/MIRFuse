...
class ImagePair:
    ...
    def __init__(self, ir_path: Path, vi_path: Path):
        ...
        self.vi_3_t = kornia.utils.image_to_tensor(self.vi_c / 255.).float()
    ...
    def save_fus(self, path: Path, fus: Tensor, color: bool = False):
        ...
        fus = kornia.utils.tensor_to_image(fus.squeeze().cpu())
    ...
    def save_lab(self, path: Path, fus: Tensor, color: bool = False):
        ...
        image = Image.fromarray(np.uint8(fus))
        save_path = path
...
class ImageFuse:
    ...
    def __init__(self, fuse_path: Path):
        """
        Load infrared and visible image pair.
        Args:
            ir_path: infrared image path
            vi_path: visible image path
        """
        self.fuse_c = cv2.imread(str(fuse_path), cv2.IMREAD_GRAYSCALE)
        # self.fuse_c = cv2.imread(str(fuse_path), cv2.IMREAD_COLOR)

        self.fuse_t = kornia.utils.image_to_tensor(cv2.cvtColor(self.fuse_c, cv2.COLOR_BGR2GRAY) / 255.).float()
